...
func TestPreviewTunRouteMatchesGitHubDestinationBinding(t *testing.T) {
	t.Parallel()
  ...
  "routing": {
	...
	runtimeConfig, err := buildTunRuntimeConfig(baseConfig, &TunFeatureSettings{
		...
		ProtectDomains: []string{"full:localhost"},
		DestinationBindings: []TunDestinationBinding{
			{Preset: string(TunDestinationBindingPresetGitHub), NodeID: "github-node"},
		},
	}, []NodeRecord{
		{ID: "general-node", URI: mustGenerateTunTestURI(t, "203.0.113.41")},
		{ID: "github-node", URI: mustGenerateTunTestURI(t, "203.0.113.42")},
	})
	if err != nil {
		t.Fatalf("build tun runtime config: %v", err)
	}
	...
	if result.OutboundTag != "pool-active-github-node" {
		t.Fatalf("expected GitHub destination binding to select github-node, got %+v", result)
	}
}
